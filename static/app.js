...
const groups = [
  ...
  ['policy','AI 回复策略','默认关闭，人工参与后必须手动恢复',[
    ['auto_reply_enabled','全局自动回复','checkbox','','开启前，各渠道入站、历史、手动文本及 OpenAI 检查必须通过。',true],
...
function field(f) {
  ...
  let control='';
  if(type==='readonly')control=`<div class="inline-value">${key==='integration_profile'?'Freshchat · 新版 Omni Ticket 未实现':key==='store'?'false（固定）· 不等于供应商零留存':esc(S.settings.webhook_path)}</div>`;
...
async function refreshDiscovery() {
  ...
  const p=d.enrollment;
  root.innerHTML=d.bindings.map(b=>`<div class="advanced-group"><h3>${esc(b.channel)} · ${b.status==='starting'?'正在读取历史并检查 OpenAI':b.status==='failed'?'启动失败':b.enabled?'AI 自动回复中':b.paused?'人工接管中':'自动回复已暂停'}</h3><p class="mono-block">客户 ID：${esc(b.user_id)}<br>会话 ID：${esc(b.conversation_id)}<br>平台来源：${esc(b.source)}</p>${b.error?`<p class="error-text">${esc(b.error)}</p>`:''}<div class="actions">${b.status==='starting'?`<button type="button" class="takeover-start" data-channel="${esc(b.channel)}" ${b.paused?'disabled':''}>${b.paused?'已暂停，检查完成后保持人工':'转人工，不自动回复'}</button>`:`<label class="check-label"><input class="quick-ai-toggle" data-channel="${esc(b.channel)}" type="checkbox" role="switch" ${b.enabled?'checked':''} ${b.status==='failed'?'disabled':''}>${esc(b.channel)} AI 自动回复</label>`}<a href="/conversations?conversation=${b.local_id}">打开会话，手动回复 →</a></div></div>`).join('')+`${p?`<h3>${p.status==='waiting'?'等待 '+esc(p.channel)+' 测试码':'识别失败'}</h3>${p.error?`<p class="error-text">${esc(p.error)}</p>`:`<p>用自己的 ${esc(p.channel)} 向平台已接通的客服账号发送下面整段文字，有效至 ${fmtTime(p.expires)}。</p><label for="test-code">一次性测试码<input id="test-code" readonly value="${esc(p.code)}"></label><button id="copy-test-code" type="button" class="small">复制测试码</button>`}`:''}<div class="field test-targets"><label for="discovery-channel">添加或更换测试账号的渠道</label><select id="discovery-channel"><option>WhatsApp</option><option>WeChat</option><option>Webchat</option></select></div><p class="readonly-note">先配置 Webhook。收到5分钟内有效的随机测试码后，自动绑定并调用模型回复。每个渠道一个账号，可同时测试。切勿把测试码发给真实客户；更换同渠道账号会替换原账号。</p><div class="actions"><button type="button" id="start-discovery" class="primary">${p?'重新生成测试码':'开启识别并自动回复'}</button>${p||d.bindings.length?'<button type="button" id="stop-discovery" class="small">结束所有测试</button>':''}</div><p class="readonly-note">各渠道可独立切换人工；待发送 AI 将取消，已提交请求无法撤回。新增账号时重新检查已绑定渠道，期间暂缓 AI。测试启用不代表工作台显示、客户送达或媒体能力已通过验收。</p>`;
  $('#discovery-channel').value=p?.channel||(['WhatsApp','WeChat'].find(ch=>!d.bindings.some(b=>b.channel===ch))||'WhatsApp');
  buttonAction($('#start-discovery'),async()=>{const channel=$('#discovery-channel').value;await saveSettings();await api('/api/test-discovery',{method:'POST',body:{channel,confirm_auto_reply:true}});await settingsPage();toast('请用自己的账号发送新生成的测试码');});
  buttonAction($('#copy-test-code'),async()=>{await navigator.clipboard.writeText(p.code);toast('测试码已复制');});
  buttonAction($('#stop-discovery'),async()=>{await api('/api/test-discovery',{method:'DELETE',body:{}});await settingsPage();toast('已结束所有测试；已提交请求无法撤回');});
  ...
  $$('.takeover-start').forEach(b=>buttonAction(b,()=>switchMode(b.dataset.channel,false)));
...
function sectionExtra(group) {
  ...
  if(group==='policy')return `<p class="readonly-note">人工接管后恢复：固定为手动。启用时记录消息时间边界，不补发历史回复。外发结果不明不会自动重试。</p>`;
...
function renderInspector() {
  ...
  $('#inspector').innerHTML=`<div class="inspector-section"><h3>AI 助理 ${pill('',c.mode==='auto'?'自动模式':c.mode==='manual'?'人工接管':'已关闭')}</h3><p class="muted">${c.mode==='manual'?'待发送 AI 已取消。恢复需明确操作。':'仅新客户公开消息可触发自动回复。'}</p><div class="actions"><button id="ai-preview" class="small">生成预览</button><button id="manual-mode" class="small">人工接管</button><button id="auto-mode" class="small">恢复 AI</button></div>${generated?`<div class="job"><small>最近计划 · ${esc(generated.result.model)} · ${generated.result.elapsed_ms} ms</small><small>用量 ${generated.result.usage?.total_tokens??'未知'} tokens · 上下文 ${generated.result.context?.count??0}/${generated.result.context?.total??0}${generated.result.context?.truncated?' · 已截断':''}</small><small class="mono">${esc(generated.result.context?.first)} → ${esc(generated.result.context?.last)}</small><details class="job-details"><summary>查看拟发送计划与请求 ID</summary><pre>${esc(JSON.stringify(generated.result,null,2))}</pre></details>${generated.result.plan.ticket_reason?`<p class="warning-text">工单建议：${esc(generated.result.plan.ticket_reason)}</p>`:''}</div>`:''}</div><div class="inspector-section"><h3>跟进工单</h3>${d.tickets.length?d.tickets.map(t=>`<p>${t.url?`<a href="${esc(t.url)}" target="_blank" rel="noopener noreferrer">Ticket #${esc(t.ticket_id)} ↗</a>`:pill(t.state)}<small> ${t.status?'状态 '+esc(t.status):''}</small></p>`).join(''):'<p class="muted">尚未关联工单</p>'}<div class="actions"><button id="create-ticket" class="small">创建 / 复用工单</button></div></div><div class="inspector-section"><h3>处理任务</h3>${d.jobs.length?d.jobs.slice(0,20).map(jobHTML).join(''):'<p class="muted">暂无处理任务</p>'}<details class="job-details"><summary>精简运行记录</summary>${d.logs.map(l=>`<div class="log-row"><time>${fmtTime(l.created)}</time><br><strong>${esc(l.event)}</strong> · ${esc(l.detail)}</div>`).join('')||'<small>暂无记录</small>'}</details></div>`;
