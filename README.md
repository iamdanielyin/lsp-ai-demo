...
4. **单会话控制 AI。** 打开自动出现的新会话，确认历史同步完成后，右侧默认显示 AI 关闭；点击“恢复 AI”才把最近可用历史交给 OpenAI 并处理新消息。手动回复或“人工接管”会取消未发送 AI；另一个会话不受影响。
...
8. **自动回复与验收。** 每个自动出现的会话默认 AI 关闭，只有会话页明确“恢复 AI”才处理新的客户消息；模型上下文使用该会话最近可用的公开历史，按预算截断。矩阵仍保留手动文本、工作台显示、客户送达的真实未验证状态；AI 媒体仍须逐渠道通过对应类型三层验证。
