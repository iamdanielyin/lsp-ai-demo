...
3. **短时监听并点选会话。** 在“测试账号与人工接管”选择 WhatsApp 或 WeChat，点击“监听下一批会话”。用自己的测试账号发送任意一条普通消息，5分钟内 Demo 只显示脱敏的会话 ID、客户 ID、来源和时间，不保存正文、不调用 AI。点选候选后才读取完整历史、写入客户白名单并开始同步；无需手工查询 ID。
4. **同时测试两个渠道。** WhatsApp 点选完成后，再选择 WeChat 监听并点选第二个候选。两个渠道各自保存会话和 AI/人工状态；选中会话默认 AI 关闭，在消息页右侧单独点击“恢复 AI”。手动回复或“人工接管”会取消未发送 AI。结束所有测试会清空范围；固定昵称和固定标记不能绑定。
...
8. **自动回复与验收。** 候选点选是管理员主动授权的限定账号试运行：历史和 OpenAI 检查完成后仍保持 AI 关闭，只有会话页明确“恢复 AI”才处理新的客户消息。矩阵仍保留手动文本、工作台显示、客户送达的真实未验证状态；历史不逐条补发，AI 媒体仍须逐渠道通过对应类型三层验证。
