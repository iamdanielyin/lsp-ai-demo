...
6. 先在 Demo 选择测试渠道并点击“开启识别并自动回复”，再用自己的账号发送页面生成的随机码。验签成功且精确匹配时自动获取 ID 和绑定；其余范围外事件忽略。识别后在“最近事件”核对消息 ID。
7. 每渠道可绑定一个测试账号，WhatsApp 和 WeChat 分别发各自的码并同时测试。所选渠道与真实事件的 `message_source` 建立映射；已知映射冲突或来源缺失时拒绝启动。不要凭 `channel_id` 推断 WhatsApp 号码。指定 WeChat 时必须验证实际连接器。
8. 在测试范围内关闭其他机器人、自动回复或重复 Webhook 消费，并核实人工分配。Webhook 是消息到达后的异步通知，不会在平台接收前拦截消息，也无法同步阻止平台内部机器人。

## 第一轮手动闭环

先填写连接信息并选择 Agent，配置公网回调与验签公钥。使用测试码流程可直接限定账号启动 AI 文本：读取真实历史并检查 OpenAI，通过后自动问候；无需先手动查 ID。这里是管理员主动授权的试运行，不自动声明手动文本、工作台展示或客户送达通过。每个渠道的开关可切换人工，手动回复也会暂停该渠道账号 AI。
