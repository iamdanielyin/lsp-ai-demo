...
6. 先在 Demo 选择测试渠道并点击“监听下一批会话”，再用自己的账号发送任意一条普通消息。验签成功后只显示脱敏候选（会话 ID、客户 ID、来源、时间）；点选候选后才读取历史、建立白名单和同步任务，未点选消息正文不落库、不调用 AI。
7. 每渠道可绑定一个测试账号，WhatsApp 和 WeChat 分别监听并点选。所选渠道与真实事件的 `message_source` 建立映射；已知映射冲突或来源缺失时拒绝选择。不要凭 `channel_id` 推断 WhatsApp 号码。指定 WeChat 时必须验证实际连接器。
8. 在测试范围内关闭其他机器人、自动回复或重复 Webhook 消费，并核实人工分配。Webhook 是消息到达后的异步通知，不会在平台接收前拦截消息，也无法同步阻止平台内部机器人。

## 第一轮手动闭环

先填写连接信息并选择 Agent，配置公网回调与验签公钥。监听并点选后 Demo 自动取得真实 ID、同步历史并检查配置；会话默认 AI 关闭，管理员在消息页单独点击“恢复 AI”才启动新消息自动回复。这里是管理员主动授权的试运行，不自动声明手动文本、工作台显示或客户送达通过。每个渠道的开关可切换人工，手动回复也会暂停该渠道账号 AI。
...
白名单为空且未开启测试码识别时，所有消息事件均忽略；点击“结束所有测试”清空范围和识别码并取消待处理任务。平台全量订阅的请求仍会到达 Demo，应用过滤不能代替网络入口限流或平台订阅过滤。当前没有全租户吞吐保证。
