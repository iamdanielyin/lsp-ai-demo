...
- `queued` / `pending` 且到期的任务可恢复，包括随机码绑定后的 `activate_test`：重新核实当前测试授权，读取已绑定账号历史并检查模型；结束测试或配置变更后旧启动任务不能恢复自动回复。启动时 `generating` 恢复排队；提交阶段 `sending` 转为 `unknown`。生成计划与待发送条目原子保存。
...
- 缓存清理按 `local_retention_days` 清理本地历史，保留活动任务、自动模式会话和未核验外发依赖。清理后将会话标记需重新全量同步；不删除平台历史。
