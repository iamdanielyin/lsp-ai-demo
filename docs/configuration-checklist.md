...
4. Demo 验签后从事件取得客户/会话 ID，自动创建本地会话并同步历史。打开会话后默认 AI 关闭，明确点击“恢复 AI”才调用 OpenAI。无需手填两个 ID。
5. 再用 WeChat 发送普通消息，第二个真实会话会自动出现。两个渠道可同时测试；不合并客户 ID 或历史。
6. 每个会话都有 **恢复 AI** / **人工接管** 操作；也可直接在消息页手动回复，或由 Omni 坐席回复触发暂停。另一会话继续运行。恢复后只处理新消息，已提交平台的请求无法撤回。
...
自动收集按已选坐席归属过滤。若某种 Freshchat 载荷没有坐席归属字段，Demo 先展示公开客户会话，再通过会话详情核实归属；其他坐席的会话在读取历史前停止并移出列表。若平台始终不提供归属，需管理员核对正确会话后再恢复 AI。平台全量投递时，其他事件仍可能到达 Demo 并验签；应用过滤不等于网络隔离或已通过压力测试。测试前协调目标会话上的既有机器人和自动回复。
