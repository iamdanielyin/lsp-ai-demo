...
3. 在 **测试账号与人工接管** 选择 WhatsApp，点击 **监听下一批会话**。用自己的 WhatsApp 向客户已接通的客服账号发送任意普通消息，5分钟内在候选列表点选。
4. Demo 验签后显示脱敏客户/会话候选；点选后才绑定、同步历史并检查 OpenAI。会话默认 AI 关闭，进入消息页后明确点击“恢复 AI”。无需手填两个 ID。失败在同一区域显示。
5. 再选择 WeChat 开启监听，用自己的 WeChat 发送普通消息并点选候选。两个渠道各一个账号，可同时测试；不合并客户 ID 或历史。已绑定的其他渠道账号保留。
6. 每个选中会话都有 **恢复 AI** / **人工接管** 操作，关闭即人工接管；也可直接在消息页手动回复，或由 Omni 坐席回复触发暂停。另一渠道继续运行。恢复后只处理新消息，已提交平台的请求无法撤回。
