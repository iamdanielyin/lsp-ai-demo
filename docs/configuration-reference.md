...
- “保存”不向客户发送；“保存并检查OpenAI”有真实模型用量；“开启识别并自动回复”授权测试码匹配账号自动回复。
...
这些是运行状态，不是要求用户填写的额外设置项。代码在加密 `meta.test_discovery` 中保存。

| 字段 / 动作 | 说明 |
| --- | --- |
| `channel` | 开始识别时选择WhatsApp或WeChat；也保留Webchat选项，但不能替代WeChat验收 |
| `confirm_auto_reply` | 点击“开启识别并自动回复”提交true，授权匹配账号生成并外发测试回复 |
| `code` / `expires` | 服务端随机生成，5分钟有效；同一时刻一个等待中的码，新码替换旧等待码 |
